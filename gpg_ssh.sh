...
function gpg-generate() {
  ...
  sed -i "s/EXPIRE/${EXPIRE:-$((`date -u +%Y` + 1))0101T000000}/" $config
  sed -i "s/KEYLGNTH/${KEYLGNTH:-3072}/" $config
  sed -i "s/REALNAME/${REALNAME:-Example Name}/" $config

function enc-edit() {
	trap '/usr/bin/rm $TMP/$$' exit 

	gpg -d "$1" 1> $TMP/$$
	$EDITOR $TMP/$$ >/dev/tty 
	gpg --batch --yes -ao "$1" -se -r 9041B81299B393998B7185690B8D3653612CA210 $TMP/$$ 
	/usr/bin/rm $TMP/$$ 2>/dev/null
}
